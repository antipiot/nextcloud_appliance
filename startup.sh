...
docker run -d --name=nextcloud --restart unless-stopped -p $https:443 --link $dbhostname PUID=$uid PGID=$gid -e TZ=Europe/Geneva -v $rootdatafolderc/config:/config -v $rootdatafolder/data:/data linuxserver/nextcloud
